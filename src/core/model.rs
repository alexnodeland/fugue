...
use crate::core::distribution::{LogF64, DistributionF64};
pub enum Model<A> {
  Pure(A),
  SampleF { addr: Address, dist: Box<dyn DistributionF64>, k: Box<dyn FnOnce(f64)->Model<A> + Send + 'static> },
  ObserveF { addr: Address, dist: Box<dyn DistributionF64>, value: f64, k: Box<dyn FnOnce(()) -> Model<A> + Send + 'static> },
  FactorF { logw: LogF64, k: Box<dyn FnOnce(()) -> Model<A> + Send + 'static> },
}
/// Lift a value into the model.
pub fn pure<A>(a:A)->Model<A>{Model::Pure(a)}
/// Create a sampling site.
pub fn sample(addr: Address, dist: impl DistributionF64 + 'static)->Model<f64>{ Model::SampleF{addr, dist: Box::new(dist), k: Box::new(pure)} }
/// Create an observe (condition) site with a fixed value.
pub fn observe(addr: Address, dist: impl DistributionF64 + 'static, value: f64)->Model<()> { Model::ObserveF{addr, dist: Box::new(dist), value, k: Box::new(pure)} }
...
impl<A: 'static> ModelExt<A> for Model<A> { fn bind<B>(self, k: impl FnOnce(A)->Model<B> + Send + 'static)->Model<B>{ match self {
  Model::Pure(a)=>k(a),
