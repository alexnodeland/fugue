...
pub enum Model<A> {
  Pure(A),
  SampleF { addr: Address, dist: Normal, k: Box<dyn FnOnce(f64)->Model<A> + Send + 'static> },
  ObserveF { addr: Address, dist: Normal, value: f64, k: Box<dyn FnOnce(()) -> Model<A> + Send + 'static> },
