...
use rand_distr::{Distribution as RandDistr, Normal as RDNormal, LogNormal as RDLogNormal, Exp as RDExp};
pub type LogF64 = f64;
...
impl DistributionF64 for Normal {
  fn sample(&self, rng: &mut dyn RngCore) -> f64 { RDNormal::new(self.mu, self.sigma).unwrap().sample(rng) }
  fn log_prob(&self, x: f64) -> LogF64 {
...
#[derive(Clone, Copy, Debug)]
pub struct Uniform { pub low: f64, pub high: f64 }
impl DistributionF64 for Uniform {
  fn sample(&self, rng: &mut dyn RngCore) -> f64 { Rng::gen_range(rng, self.low..self.high) }
  fn log_prob(&self, x: f64) -> LogF64 {
    if x < self.low || x > self.high { f64::NEG_INFINITY } else { -(self.high - self.low).ln() }
  }
}

#[derive(Clone, Copy, Debug)]
pub struct LogNormal { pub mu: f64, pub sigma: f64 }
impl DistributionF64 for LogNormal {
  fn sample(&self, rng: &mut dyn RngCore) -> f64 { RDLogNormal::new(self.mu, self.sigma).unwrap().sample(rng) }
  fn log_prob(&self, x: f64) -> LogF64 {
...
impl DistributionF64 for Exponential {
  fn sample(&self, rng: &mut dyn RngCore) -> f64 { RDExp::new(self.rate).unwrap().sample(rng) }
  fn log_prob(&self, x: f64) -> LogF64 { if x < 0.0 { f64::NEG_INFINITY } else { self.rate.ln() - self.rate * x } }
}
