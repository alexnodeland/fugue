...
fn compute_autocorrelation(samples: &[f64], max_lag: usize) -> Vec<f64> {
    ...
    for lag in 1..=max_lag {
        ...
        let covariance: f64 = centered[..n-lag].iter()
...
pub fn geweke_diagnostic(chain: &[f64]) -> f64 {
    ...
    let var1 = first_part.iter().map(|&x| (x - mean1).powi(2)).sum::<f64>() / (first_part.len() - 1) as f64;
    let var2 = last_part.iter().map(|&x| (x - mean2).powi(2)).sum::<f64>() / (last_part.len() - 1) as f64;
...
mod mcmc_tests {
    ...
    fn test_effective_sample_size() {
        ...
        let random: Vec<f64> = (0..100).map(|i| (i as f64).sin() * (i as f64).cos()).collect();
