...
pub fn test_conjugate_normal_model<R: Rng>(
    ...
    let posterior_mu = posterior_variance * (prior_precision * prior_mu + likelihood_precision * observation);
    ...
    pub fn print_summary(&self) {
        match self {
            ValidationResult::Success { 
                ...
                println!("  True posterior: N({:.4}, {:.4})", posterior_mu, posterior_sigma);
                println!("  Sample estimates: N({:.4}, {:.4})", sample_mean, sample_sigma);
                println!("  Mean error: {:.6} ({})", mean_error, if *mean_within_bounds { "PASS" } else { "FAIL" });
                println!("  Var error: {:.6} ({})", var_error, if *var_within_bounds { "PASS" } else { "FAIL" });
                println!("  ESS: {:.1} ({})", effective_sample_size, if *ess_adequate { "PASS" } else { "FAIL" });
                println!("  Overall: {}", if self.is_valid() { "PASS" } else { "FAIL" });
            }
...
mod validation_tests {
    ...
    fn test_normal_distribution() {
        ...
        let reference: Vec<f64> = (0..1000).map(|_| {
            let u1: f64 = rng.gen();
            let u2: f64 = rng.gen();
