...
pub fn estimate_elbo<A, R: Rng>(rng: &mut R, model_fn: impl Fn() -> Model<A>, num_samples: usize) -> f64 {
  let mut total = 0.0;
  for _ in 0..num_samples {
    let (_a, prior_t) = run(PriorHandler{rng, trace: Trace::default()}, model_fn());
    let (_a2, scored) = run(ScoreGivenTrace{base: prior_t.clone(), trace: Trace::default()}, model_fn());
