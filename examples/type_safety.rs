...
fn natural_type_system() {
    ...
    println!("👥 Customers: {} arrivals, {}min service (type: u64)", arrivals, service_time);
    ...
    println!("🛒 Customer chose: {} (index: {}, type: usize)", products[choice], choice);
...
fn compile_time_safety_demo() {
    ...
    println!("📊 Sample: coin={}, events={}, category={}, value={:.3}", 
...
fn safe_array_indexing() {
    ...
    for trial in 1..=5 {
        ...
        println!("Trial {}: Selected method '{}' (index {})", 
...
fn parameter_validation_demo() {
    ...
    match Categorical::new(vec![0.5, 0.6]) { // Doesn't sum to 1
...
fn type_safe_observations() {
    ...
    println!("📊 Model executed successfully with {} addresses", 
             trace.choices.len());
    println!();
}
...
fn advanced_type_composition() {
    ...
    for (i, ((&size, &success), &category)) in 
        sizes.iter().zip(successes.iter()).zip(categories.iter()).enumerate() {
        println!("   Group {}: {} trials, {} successes, category {}", 
...
mod tests {
    ...
    fn test_hierarchical_composition() {
        ...
        for (i, (&size, &success)) in sizes.iter().zip(successes.iter()).enumerate() {
            assert!(success <= size, "Group {}: {} successes > {} trials", i, success, size);
        }
