...
fn main() {
    ...
    for _ in 0..20 {
        ...
        assert!(test_trace.total_log_weight().is_finite(), 
                "Trace should have finite log-weight");
        assert!(test_trace.log_likelihood.is_finite(), 
    ...
    println!("   - Sample mean: {:.3} (expected ~0.7 for Beta-Binomial)", sample_mean);
    ...
    println!("   - Retrieved value: {:?}", safe_trace.get_f64(&addr!("param")));
    ...
    println!("   - Score trace log-weight: {:.3}", score_trace.total_log_weight());
    ...
    if !chain1_samples.is_empty() && !chain2_samples.is_empty() {
        ...
        println!("   - Chain 1: {} samples, ESS = {:.1}", chain1_samples.len(), ess1);
        println!("   - Chain 2: {} samples, ESS = {:.1}", chain2_samples.len(), ess2);
    ...
    for (addr, choice) in &complex_trace.choices {
        ...
        address_analysis.entry(category).or_insert(Vec::new()).push((addr_str, choice.logp));
    }
    ...
    for (category, addresses) in address_analysis {
        ...
        for (addr, logp) in addresses.iter().take(3) { // Show first 3
    ...
    fn test_model_basic_properties<F, T>(model_fn: F, expected_choice_count: usize, description: &str) 
    where F: Fn() -> Model<T> {
        let mut rng = thread_rng();
        ...
        assert!(trace.total_log_weight().is_finite(), "Log-weight should be finite");
        assert_eq!(trace.choices.len(), expected_choice_count, "Choice count mismatch");
...
mod tests {
    ...
    fn test_performance_diagnostics() {
        ...
        let large_model = || plate!(i in 0..50 => {
