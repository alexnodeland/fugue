...
fn generate_mixture_data(n: usize, components: &[(f64, f64, f64)], seed: u64) -> (Vec<f64>, Vec<usize>) {
    ...
    let cumulative_weights: Vec<f64> = weights.iter().scan(0.0, |acc, &w| { *acc += w; Some(*acc) }).collect();
    
    for _ in 0..n {
        // Sample component
        let u: f64 = rng.gen();
        let component = cumulative_weights.iter().position(|&cw| u <= cw).unwrap_or(components.len() - 1);
...
fn gaussian_mixture_demo() {
    ...
    println!("📊 Generated {} data points from {} true components", data.len(), true_components.len());
    for (i, (weight, mu, sigma)) in true_components.iter().enumerate() {
        println!("   - Component {}: π={:.1}, μ={:.1}, σ={:.1}", i+1, weight, mu, sigma);
    }
    
    let n_true_labels: Vec<usize> = (0..true_components.len()).map(|k| 
        true_labels.iter().filter(|&&label| label == k).count()
    ).collect();
    
    for (k, count) in n_true_labels.iter().enumerate() {
        println!("   - True cluster {}: {} observations ({:.1}%)", 
                k+1, count, 100.0 * *count as f64 / data.len() as f64);
    }
    ...
    if !valid_samples.is_empty() {
        println!("✅ MCMC completed with {} valid samples", valid_samples.len());
        ...
        println!("   - Component 1: π̂={:.2}, μ̂={:.1}, σ̂={:.1}", mean_pi1, mean_mu1, mean_sigma1);
        println!("   - Component 2: π̂={:.2}, μ̂={:.1}, σ̂={:.1}", 1.0 - mean_pi1, mean_mu2, mean_sigma2);
...
fn multivariate_mixture_model(data: Vec<Vec<f64>>) -> Model<(f64, f64, f64, f64, f64, f64, f64, f64)> {
    prob! {
        // Mixing weight
        let pi1 <- sample(addr!("pi1"), fugue::Beta::new(1.0, 1.0).unwrap());
...
fn multivariate_mixture_demo() {
    ...
    println!("📊 Generated {} 2D data points from {} components", data.len(), true_components.len());
    for (i, (weight, ref mu_vec, sigma)) in true_components.iter().enumerate() {
        println!("   - Component {}: π={:.1}, μ=[{:.1}, {:.1}], σ={:.1}", 
                i+1, weight, mu_vec[0], mu_vec[1], sigma);
    }
    ...
    if !valid_samples.is_empty() {
        ...
        println!("   - Component 1: π̂={:.2}, μ̂=[{:.1}, {:.1}]", mean_pi1, mean_mu1_0, mean_mu1_1);
        println!("   - Component 2: π̂={:.2}, μ̂=[{:.1}, {:.1}]", 1.0 - mean_pi1, mean_mu2_0, mean_mu2_1);
...
fn mixture_of_experts_model(x_data: Vec<f64>, y_data: Vec<f64>) -> Model<(f64, f64, f64, f64, f64, f64)> {
    prob! {
...
fn mixture_of_experts_demo() {
    ...
    println!("📊 Generated {} (x,y) points with region-specific relationships", x_data.len());
    ...
    if !valid_samples.is_empty() {
        ...
        let intercept1_samples: Vec<f64> = valid_samples.iter().map(|(params, _)| params.0).collect();
        let slope1_samples: Vec<f64> = valid_samples.iter().map(|(params, _)| params.1).collect();
        let sigma1_samples: Vec<f64> = valid_samples.iter().map(|(params, _)| params.2).collect();
        
        let intercept2_samples: Vec<f64> = valid_samples.iter().map(|(params, _)| params.3).collect();
        ...
        let mean_intercept1 = intercept1_samples.iter().sum::<f64>() / intercept1_samples.len() as f64;
        let mean_slope1 = slope1_samples.iter().sum::<f64>() / slope1_samples.len() as f64;
        let mean_sigma1 = sigma1_samples.iter().sum::<f64>() / sigma1_samples.len() as f64;
        
        let mean_intercept2 = intercept2_samples.iter().sum::<f64>() / intercept2_samples.len() as f64;
        let mean_slope2 = slope2_samples.iter().sum::<f64>() / slope2_samples.len() as f64;
        let mean_sigma2 = sigma2_samples.iter().sum::<f64>() / sigma2_samples.len() as f64;
        
        println!("   - Expert 1 [Left (x < 0)]: intercept={:.2}, slope={:.2}, σ={:.2}", 
                mean_intercept1, mean_slope1, mean_sigma1);
        println!("   - Expert 2 [Right (x ≥ 0)]: intercept={:.2}, slope={:.2}, σ={:.2}", 
...
fn dirichlet_process_mixture_model(data: Vec<f64>) -> Model<(f64, f64, f64, f64, f64, f64, f64, usize)> {
    prob! {
...
fn dirichlet_process_mixture_demo() {
    ...
    println!("📊 Generated {} data points from {} unknown components", data.len(), true_components.len());
    ...
    if !valid_samples.is_empty() {
        ...
        let active_counts: Vec<usize> = valid_samples.iter()
            .map(|(params, _)| params.7)
            .collect();
        ...
        let mode_active = {
            ...
            counts.iter().enumerate().max_by_key(|(_, &count)| count).unwrap().0
        };
...
fn hidden_markov_model_demo() {
    ...
    println!("📊 Generated {} observations from switching regime process", hmm_data.len());
    ...
    if !valid_samples.is_empty() {
        println!("✅ HMM MCMC completed with {} valid samples", valid_samples.len());
        ...
        println!("   - State 0: μ̂={:.2}, σ̂={:.2} ({} volatility)", mean_mu0, mean_sigma0, volatility_type0);
        println!("   - State 1: μ̂={:.2}, σ̂={:.2} ({} volatility)", mean_mu1, mean_sigma1, volatility_type1);
...
fn mixture_model_selection_demo() {
    ...
    println!("📊 Generated data from {} true components", true_components.len());
    println!("   Comparing single Gaussian vs 2-component mixture...");
    
    // Single Gaussian model
    let single_gaussian_model = move |data: Vec<f64>| prob! {
    ...
    let single_valid: Vec<_> = single_samples.iter()
        .filter(|(_, trace)| trace.total_log_weight().is_finite())
        .collect();
    
    let mixture_valid: Vec<_> = mixture_samples.iter()
        .filter(|(_, trace)| trace.total_log_weight().is_finite())
        .collect();
    
    if !single_valid.is_empty() && !mixture_valid.is_empty() {
        let single_loglik = single_valid.iter()
            .map(|(_, trace)| trace.total_log_weight())
            .sum::<f64>() / single_valid.len() as f64;
        
        let mixture_loglik = mixture_valid.iter()
            .map(|(_, trace)| trace.total_log_weight())
            .sum::<f64>() / mixture_valid.len() as f64;
        
        println!("\n🏆 Model Comparison Results:");
        println!("   Model               | Samples | Log-Likelihood");
        println!("   --------------------|---------|---------------");
        println!("   Single Gaussian     | {:7} | {:13.1}", single_valid.len(), single_loglik);
        println!("   2-Component Mixture | {:7} | {:13.1}", mixture_valid.len(), mixture_loglik);
...
fn cluster_diagnostics_demo() {
    ...
    if !valid_samples.is_empty() {
        println!("✅ Fitted mixture model with {} samples", valid_samples.len());
        ...
        println!("   - Accuracy: {:.2} ({} correct out of {})", accuracy, correct, data_for_diagnostics.len());
