...
use fugue::runtime::memory::{TracePool, TraceBuilder, PooledPriorHandler};
...
impl<H: Handler> Handler for RobustProductionHandler<H> {
    ...
    fn finish(self) -> Trace {
        ...
        println!("   - Errors encountered: {}/{}", self.error_count, self.max_errors);
        println!("   - Circuit breaker status: {}", 
                 if self.circuit_breaker_open { "OPEN (fallback mode)" } else { "CLOSED (normal)" });
...
impl ConfigurableModelRunner {
    ...
    fn run_inference(&mut self) -> Result<(f64, bool), String> {
        ...
        let result = if self.config.environment == "production" {
            ...
            let robust_handler = RobustProductionHandler::new(
                base_handler, 
        ...
        if duration.as_millis() > self.config.max_inference_time_ms as u128 {
            ...
            return Err(format!("Inference timeout: {}ms > {}ms", 
                               duration.as_millis(), self.config.max_inference_time_ms));
        }
...
impl ProductionMetrics {
    ...
    fn get_stats(&self) -> HashMap<String, f64> {
        ...
        if self.enabled {
            ...
            stats.insert("throughput_per_second".to_string(),
                       if uptime.as_secs() > 0 {
    ...
    fn export_prometheus_metrics(&self) -> String {
        ...
        metrics.push_str("# HELP fugue_inference_duration_ms Average inference duration in milliseconds\n");
...
impl<H: Handler> MetricsHandler<H> {
    fn new(inner: H, metrics: Arc<std::sync::Mutex<ProductionMetrics>>, model_name: String) -> Self {
        Self { inner, metrics, model_name }
    }
}
...
impl ProductionHealthChecker {
    ...
    fn run_health_check(&self) -> HealthCheckResult {
        ...
        if let Ok(metrics) = self.metrics.lock() {
            ...
            details.insert("avg_inference_time_ms".to_string(), format!("{:.1}", avg_time));
        ...
        details.insert("memory_pool_size".to_string(), 
                      self.model_config.memory_pool_size.to_string());
        details.insert("circuit_breaker".to_string(), 
...
fn main() {
    ...
    println!("   - Timestamp: {:?}", health_result.timestamp.duration_since(UNIX_EPOCH).unwrap().as_secs());
    println!();
    ...
    for (temp, sensor, prob) in test_cases {
        match create_validated_model(temp, sensor, prob) {
            Ok(model) => {
                ...
                let (result, _trace) = runtime::handler::run(handler, model);
                println!("✅ Valid input: temp={:.1}, sensor={}, prob={:.2} → result=({:.1}, {})",
                         temp, sensor, prob, result.0, result.1);
            }
            Err(e) => {
                println!("❌ Invalid input: temp={:.1}, sensor={}, prob={:.2} → error: {}",
                         temp, sensor, prob, e);
            }
        }
    }
    ...
    let strategies = vec![
        ...
        ("Canary 20%", DeploymentStrategy::CanaryRelease { percentage: 0.2 }),
...
mod tests {
    ...
    fn test_deployment_strategies() {
        let canary_manager = ModelDeploymentManager::new(
            DeploymentStrategy::CanaryRelease { percentage: 0.3 }
        );
        ...
        assert!(percentage > 0.25 && percentage < 0.35, 
                "Canary percentage was {:.2}, expected ~0.30", percentage);
    }
