...
fn main() {
    ...
    let observed_flips = vec![true, false, true, true, false, true, true, false, true, true];
    ...
    println!("\n📊 Summary: {} successes out of {} flips ({:.1}%)", 
             successes, n_flips, (successes as f64 / n_flips as f64) * 100.0);
    ...
    println!("📈 Prior Distribution: Beta({}, {})", prior_alpha, prior_beta);
    ...
    println!("   - Model incorporates {} observations", observed_flips.len());
    println!();
    ...
    println!("🎯 Analytical Posterior: Beta({:.0}, {:.0})", posterior_alpha, posterior_beta);
    ...
    println!("   - 95% credible interval: approximately [{:.2}, {:.2}]", 
    ...
    if !posterior_samples.is_empty() {
        ...
        let mcmc_variance = {
            let mean = mcmc_mean;
            posterior_samples.iter()
                .map(|x| (x - mean).powi(2_i32))
                .sum::<f64>() / (posterior_samples.len() - 1) as f64
        };
        let ess = effective_sample_size(&posterior_samples);
        
        println!("✅ MCMC Results ({} effective samples from {} total):", 
                 ess as usize, posterior_samples.len());
        println!("   - MCMC mean: {:.3} (analytical: {:.3})", mcmc_mean, posterior_mean);
        println!("   - MCMC variance: {:.4} (analytical: {:.4})", mcmc_variance, posterior_variance);
        println!("   - Effective Sample Size: {:.0}", ess);
        println!("   - Agreement: {}", 
                 if (mcmc_mean - posterior_mean).abs() < 0.05 { "✅ Excellent" } else { "⚠️ Check convergence" });
    }
    ...
    let pred_within_range = predicted_successes.iter()
        .filter(|&&x| (x as i32 - successes as i32).abs() <= 2)
        .count() as f64 / n_pred_samples as f64;
    
    println!("   - Observed successes: {}", successes);
    println!("   - Predicted mean successes: {:.1}", pred_mean);
    println!("   - P(|pred - obs| ≤ 2): {:.1}%", pred_within_range * 100.0);
    ...
    println!("   - Posterior probability coin is fair: {:.1}%", prob_fair * 100.0);
    println!("   - Evidence for bias: {}", 
             if prob_fair < 0.3 { "Strong" } 
             else if prob_fair < 0.7 { "Moderate" }
             else { "Weak" });
    ...
    println!("   - Expected heads in next {} flips: {:.1} ± {:.1}", 
             future_flips, expected_heads, 1.96 * uncertainty);
