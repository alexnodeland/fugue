...
fn generate_hierarchical_data(n_groups: usize, n_per_group: usize, seed: u64) -> (Vec<Vec<f64>>, Vec<bool>, Vec<usize>) {
    let mut rng = StdRng::seed_from_u64(seed);
    let mut features = Vec::new();
    let mut labels = Vec::new();
...
fn binary_classification_demo() {
    ...
    println!("   - Positive cases: {} / {} ({:.1}%)", 
             positive_cases, labels.len(), 
    ...
    if !valid_samples.is_empty() {
        ...
        for (i, (name, true_val)) in coef_names.iter().zip(true_coefs.iter()).enumerate() {
            let coef_samples: Vec<f64> = valid_samples.iter()
                .map(|coeffs| coeffs[i])
                .collect();
            
            let mean_coef = coef_samples.iter().sum::<f64>() / coef_samples.len() as f64;
            let std_coef = {
                let variance = coef_samples.iter()
                    .map(|c| (c - mean_coef).powi(2))
                    .sum::<f64>() / (coef_samples.len() - 1) as f64;
                variance.sqrt()
            };
            
            println!("   - {}: {:.3} ± {:.3} (true: {:.1})", name, mean_coef, std_coef, true_val);
        }
        
        // Model diagnostics
        let avg_log_weight = samples.iter()
        ...
        for coeffs in valid_samples.iter().take(50) { // Use subset for speed
            let mut linear_pred = 0.0;
        ...
        let std_prob = {
            let variance = predicted_probs.iter()
                .map(|p| (p - mean_prob).powi(2))
                .sum::<f64>() / (predicted_probs.len() - 1) as f64;
            variance.sqrt()
        };
        
        println!("   - Test point [0.5, -0.8]: P(y=1) = {:.3} ± {:.3}", mean_prob, std_prob);
        if mean_prob > 0.5 {
...
fn multiclass_classification_demo() {
    ...
    for (class_id, count) in class_counts.iter().enumerate() {
        println!("   - Class {}: {} samples ({:.1}%)", 
                class_id, count, 100.0 * *count as f64 / labels.len() as f64);
    }
    ...
    for target_class in 0..3 {
        ...
        let binary_labels: Vec<bool> = labels.iter()
        ...
        println!("   - Positive cases: {} / {}", positive_cases, binary_labels.len());
        
        // Clone data for each iteration to avoid move issues
        let features_copy = features.clone();
        let model_fn = move || logistic_regression_model(features_copy.clone(), binary_labels.clone());
...
fn hierarchical_classification_demo() {
    ...
    println!("   - {} groups with {} observations each", n_groups, features.len() / n_groups);
    println!("   - Total: {} data points", features.len());
    
    // Show group-wise statistics
    for group_id in 0..n_groups {
        let group_labels: Vec<bool> = groups.iter()
            .zip(labels.iter())
            .filter_map(|(&g, &y)| if g == group_id { Some(y) } else { None })
            .collect();
        
        let positive_rate = group_labels.iter().filter(|&&x| x).count() as f64 / group_labels.len() as f64;
        println!("   - Group {}: {:.1}% positive cases", group_id, positive_rate * 100.0);
    }
    
    println!("\n🔬 Running hierarchical MCMC...");
    let model_fn = move || hierarchical_classification_model(
        features.clone(), 
        labels.clone(), 
        groups.clone()
    );
    ...
    if !valid_samples.is_empty() {
        println!("✅ Hierarchical MCMC completed with {} valid samples", valid_samples.len());
        
        // Extract global parameters
        let global_intercepts: Vec<f64> = valid_samples.iter()
            .map(|(params, _)| params.0)
            .collect();
        let slopes: Vec<f64> = valid_samples.iter()
            .map(|(params, _)| params.1)
            .collect();
        
        let mean_global_int = global_intercepts.iter().sum::<f64>() / global_intercepts.len() as f64;
        ...
        for group_id in 0..n_groups {
            let group_intercepts: Vec<f64> = valid_samples.iter()
                .map(|(params, _)| params.2[group_id])
                .collect();
            
            let mean_group_int = group_intercepts.iter().sum::<f64>() / group_intercepts.len() as f64;
...
fn model_comparison_demo() {
    ...
    {
        let intercept_features: Vec<Vec<f64>> = features.iter()
            .map(|f| vec![f[0]]) // Just intercept
            .collect();
        let labels_clone = labels.clone();
        
        let model_fn = move || logistic_regression_model(intercept_features.clone(), labels_clone.clone());
        ...
        if !valid_samples.is_empty() {
            let avg_log_lik = valid_samples.iter()
                .map(|(_, trace)| trace.total_log_weight())
                .sum::<f64>() / valid_samples.len() as f64;
    ...
    {
        let reduced_features: Vec<Vec<f64>> = features.iter()
            .map(|f| vec![f[0], f[1]]) // Intercept + first feature
            .collect();
        let labels_clone = labels.clone();
        
        let model_fn = move || logistic_regression_model(reduced_features.clone(), labels_clone.clone());
    ...
    if !results.is_empty() {
        ...
        // Find best model
        if let Some(best) = results.iter().max_by(|a, b| a.log_likelihood.partial_cmp(&b.log_likelihood).unwrap()) {
...
mod tests {
    ...
    fn test_hierarchical_model() {
        let features = vec![
            vec![1.0, 0.5], vec![1.0, -0.3],  // Group 0
            vec![1.0, 1.2], vec![1.0, -0.7],  // Group 1
        ];
    ...
    fn test_classification_mcmc() {
        ...
        let valid_samples = samples.iter()
            .filter(|(_, trace)| trace.total_log_weight().is_finite())
            .count();
