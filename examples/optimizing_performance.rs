use fugue::*;
use fugue::runtime::memory::{TracePool, TraceBuilder, PooledPriorHandler, CowTrace};
...
fn main() {
    ...
    println!("   - Pool stats - hits: {}, misses: {}", stats.hits, stats.misses);
    ...
    println!("   - Probabilities sum to: {:.10}", normalized_probs.iter().sum::<f64>());
    ...
    let safe_results: Vec<f64> = vec![1.0, 0.0, -1.0]
        .iter()
        .map(|&x| safe_ln(x))
        .collect();
    ...
    println!("   - Built trace with {} choices in {:?}", 
             constructed_trace.choices.len(), construction_time);
    println!("   - Total log weight: {:.2}", constructed_trace.total_log_weight());
    ...
    for _proposal in 0..1000 {
        ...
        proposal_trace.insert_choice(addr!("mu"), Choice {
            addr: addr!("mu"),
    ...
    impl TraceMetrics {
        fn from_trace(trace: &Trace) -> Self {
            ...
            Self { num_choices, log_weight, is_valid, memory_size_estimate }
        }
    }
    ...
    println!("   - Memory estimate: {} bytes", metrics.memory_size_estimate);
    ...
    println!("   - Memory efficiency: {:.1}% hit ratio", batch_stats.hit_ratio());
    ...
    for &scale in &test_scales {
        ...
        println!("   Scale {:.0e}: log_sum_exp={:.4}, log1p_exp={:.4}", 
...
mod tests {
    ...
    fn test_numerical_stability() {
        ...
        assert!(result.is_finite(), "log_sum_exp should handle extreme values");
        
        // Test normalization
        let normalized = normalize_log_probs(&extreme_vals);
        let sum: f64 = normalized.iter().sum();
        assert!((sum - 1.0).abs() < 1e-10, "Normalized probabilities should sum to 1");
        
        // Test weighted computation
        let weights = vec![0.5, 0.3, 0.2];
        let weighted_result = weighted_log_sum_exp(&extreme_vals, &weights);
        assert!(weighted_result.is_finite(), "Weighted log_sum_exp should be finite");
    }
