...
fn main() {
    ...
    println!("📈 Log-density at x=0: {:.3} (peak of standard normal)", log_density);
    ...
    println!("🎯 Log-probability of exactly 3 events: {:.3}", prob_3_events);
    ...
    println!("🎲 Categorical choice (weights: 0.3, 0.5, 0.2): {}", options[selected]);
    ...
    println!("🌡️  True temperature: {:.2}°C → Measured: {:.2}°C", true_temperature, measured_temp);
    ...
    println!("🚪 Expected arrivals: 2.5/hour → Actual: {} arrivals", hourly_arrivals);
    ...
    println!("📋 Evaluating {} observations under Normal(μ=100.0, σ=15.0):", observations.len());
    ...
    if total_log_prob > -700.0 { // Avoid underflow
