...
use fugue::runtime::{
    ...
    trace::{Trace, Choice, ChoiceValue},
...
fn basic_trace_inspection() {
    ...
    for (addr, choice) in &trace.choices {
        println!("   - {}: {:?} (logp: {:.4})", addr, choice.value, choice.logp);
    }
    println!();
    ...
    println!("   - Category (usize): {:?}", trace.get_usize(&addr!("category")));
    println!("   - Measurement (f64): {:?}", trace.get_f64(&addr!("measurement")));
    
    let (coin, count, category, measurement) = result;
    println!("   - Result: coin={}, count={}, category={}, measurement={:.3}", 
...
fn replay_mechanics() {
    ...
    println!("   - Traces match: {}", trace1.total_log_weight() == trace2.total_log_weight());
    println!();
    ...
    if let Some(choice) = modified_trace.choices.get_mut(&addr!("mu")) {
        ...
        println!("   - Old logp: {:.3}", trace1.get_f64(&addr!("mu")).map(|v| 
                 Normal::new(0.0, 2.0).unwrap().log_prob(&v)).unwrap_or(0.0));
...
impl<R: rand::Rng> Handler for DebugHandler<R> {
    fn on_sample_f64(&mut self, addr: &Address, dist: &dyn Distribution<f64>) -> f64 {
        ...
        self.debug_info.push(format!(
            "SAMPLE f64 at {}: {} (logp: {:.3})", addr, value, logp
        ));
    ...
    fn on_sample_bool(&mut self, addr: &Address, dist: &dyn Distribution<bool>) -> bool {
        ...
        self.debug_info.push(format!(
            "SAMPLE bool at {}: {} (logp: {:.3})", addr, value, logp
        ));
    ...
    fn on_sample_u64(&mut self, addr: &Address, dist: &dyn Distribution<u64>) -> u64 {
        ...
        self.debug_info.push(format!(
            "SAMPLE u64 at {}: {} (logp: {:.3})", addr, value, logp
        ));
    ...
    fn on_sample_usize(&mut self, addr: &Address, dist: &dyn Distribution<usize>) -> usize {
        ...
        self.debug_info.push(format!(
            "SAMPLE usize at {}: {} (logp: {:.3})", addr, value, logp
        ));
    ...
    fn on_observe_f64(&mut self, addr: &Address, dist: &dyn Distribution<f64>, value: f64) {
        ...
        self.debug_info.push(format!(
            "OBSERVE f64 at {}: {} (logp: {:.3})", addr, value, logp
        ));
    ...
    fn on_observe_bool(&mut self, addr: &Address, dist: &dyn Distribution<bool>, value: bool) {
        ...
        self.debug_info.push(format!(
            "OBSERVE bool at {}: {} (logp: {:.3})", addr, value, logp
        ));
    ...
    fn on_observe_u64(&mut self, addr: &Address, dist: &dyn Distribution<u64>, value: u64) {
        ...
        self.debug_info.push(format!(
            "OBSERVE u64 at {}: {} (logp: {:.3})", addr, value, logp
        ));
    ...
    fn on_observe_usize(&mut self, addr: &Address, dist: &dyn Distribution<usize>, value: usize) {
        ...
        self.debug_info.push(format!(
            "OBSERVE usize at {}: {} (logp: {:.3})", addr, value, logp
        ));
...
fn custom_handler_demo() {
    ...
    println!("   - Total log-weight: {:.4}", final_trace.total_log_weight());
    println!();
...
fn memory_optimization_demo() {
    ...
    let make_model = |obs_val: f64| prob!(
    ...
    for (i, &obs) in observations.iter().enumerate() {
        ...
        println!("   Sample {}: mu={:.3}, sigma={:.3}, obs={:.1}, logp={:.3}", 
                 i + 1, result.0, result.1, obs, results[i].1.total_log_weight());
    }
    
    println!();
    println!("📊 Batch Statistics:");
    let mu_mean = results.iter().map(|((mu, _), _)| mu).sum::<f64>() / results.len() as f64;
    let sigma_mean = results.iter().map(|((_, sigma), _)| sigma).sum::<f64>() / results.len() as f64;
    let logp_mean = results.iter().map(|(_, trace)| trace.total_log_weight()).sum::<f64>() / results.len() as f64;
    ...
    println!("   - Manual trace: {} choices, total logp: {:.3}",
             demo_trace.choices.len(), demo_trace.total_log_weight());
    println!();
}
...
fn diagnostic_tools_demo() {
    ...
    println!("   - mu chain1: mean={:.3}, min={:.3}, max={:.3}", 
    ...
    println!("   - mu chain2: mean={:.3}, min={:.3}, max={:.3}", 
    ...
    println!("   - precision chain1: mean={:.3}, min={:.3}, max={:.3}", 
             precision_values1.iter().sum::<f64>() / precision_values1.len() as f64,
             precision_values1.iter().fold(f64::INFINITY, |a, &b| a.min(b)),
             precision_values1.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b)));
    ...
    println!("   - R-hat for precision: {:.4} (< 1.1 is good)", r_hat_precision);
    ...
    println!("   - mu summary: mean={:.3}, std={:.3}, q2.5={:.3}, q97.5={:.3}", 
    ...
    println!("   - Chains similar quality: {}", (avg_logp1 - avg_logp2).abs() < 0.5);
    println!();
}
...
fn advanced_debugging_demo() {
    ...
    for attempt in 1..=5 {
        ...
        println!("   Attempt {}: scale={:.6}, precision={:.6}, mu={:.3}, logp={:.3}", 
        ...
        println!("     Components: prior={:.3}, likelihood={:.3}, factors={:.3}",
    ...
    let validation_trace = Trace {
        choices: [
            ...
            (addr!("mu"), Choice {
                addr: addr!("mu"),
                value: ChoiceValue::F64(0.5),
                logp: Normal::new(0.0, 1.0 / (2.0_f64).sqrt()).unwrap().log_prob(&0.5),
            }),
        ].iter().cloned().collect(),
        log_prior: 0.0,
        log_likelihood: 0.0,
    ...
    println!("   - Validation finite: {}", val_trace.total_log_weight().is_finite());
    println!();
}
