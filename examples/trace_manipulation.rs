...
fn simple_model(obs: f64) -> Model<(f64, f64)> {
    sample(addr!("mu"), Normal{mu: 0.0, sigma: 2.0}).bind(move |mu| {
        sample(addr!("sigma"), LogNormal{mu: 0.0, sigma: 0.5}).bind(move |sigma| {
            observe(addr!("y"), Normal{mu, sigma}, obs).bind(move |_| {
                pure((mu, sigma))
            })
        })
    })
...
fn demonstrate_trace_utilities(seed: u64) {
    ...
    println!("Model: mu ~ N(0,2), sigma ~ LogN(0,0.5), y ~ N(mu,sigma) with y = {}", obs_value);
    ...
    let ((mu2, sigma2), trace2) = runtime::handler::run(
        runtime::interpreters::ReplayHandler{rng: &mut rng, base: trace1.clone(), trace: Trace::default()}, 
        model2
    );
    ...
    let ((mu3, sigma3), trace3) = runtime::handler::run(
        runtime::interpreters::ScoreGivenTrace{base: trace1.clone(), trace: Trace::default()}, 
        model3
    );
    ...
    if let Some(mu_choice) = modified_trace.choices.get_mut(&addr!("mu")) {
        ...
        mu_choice.logp = Normal{mu: 0.0, sigma: 2.0}.log_prob(new_mu);
    ...
    let ((mu4, sigma4), trace4) = runtime::handler::run(
        runtime::interpreters::ScoreGivenTrace{base: modified_trace, trace: Trace::default()}, 
        model4
    );
    ...
    println!("  Replayed (different obs): {:.4}", trace2.total_log_weight());
