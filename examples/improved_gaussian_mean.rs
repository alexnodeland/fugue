...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let prior = Normal { mu: 0.0, sigma: 5.0 };
    let likelihood = Normal { mu: args.obs, sigma: 1.0 };
    prior.validate()?;
    likelihood.validate()?;
    ...
    let posterior_mean = posterior_variance * (prior_precision * 0.0 + likelihood_precision * args.obs);
    ...
    for chain_id in 0..args.n_chains {
        ...
        // Extract mu values
        let mu_samples: Vec<f64> = chain_samples.iter()
        ...
        println!("  Chain {}: mean={:.4}, sd={:.4}, ESS={:.1}, Geweke={:.2}", 
                 chain_id + 1, chain_mean, chain_var.sqrt(), ess, geweke);
    ...
    let sample_var = all_samples.iter()
        .map(|&x| (x - sample_mean).powi(2))
        .sum::<f64>() / (all_samples.len() - 1) as f64;
    
    println!("\n=== Posterior Summary ===");
    println!("Sample mean: {:.4} (true: {:.4})", sample_mean, posterior_mean);
    println!("Sample sd: {:.4} (true: {:.4})", sample_var.sqrt(), posterior_variance.sqrt());
    println!("Mean error: {:.6}", (sample_mean - posterior_mean).abs());
    println!("SD error: {:.6}", (sample_var.sqrt() - posterior_variance.sqrt()).abs());
    ...
    println!("Prior sample: μ={:.4}, log_weight={:.4}", 
             prior_sample, prior_trace.total_log_weight());
    ...
    let smc_mean = smc_particles.iter()
        .filter_map(|p| p.trace.choices.get(&addr!("mu")))
        .filter_map(|choice| match choice.value {
            ChoiceValue::F64(mu) => Some(mu),
            _ => None,
        })
        .map(|mu| mu * smc_particles.iter().find(|p| 
    ...
    println!("SMC estimate: μ={:.4}, ESS={:.1}", 
             smc_mean, effective_sample_size(&smc_particles));
