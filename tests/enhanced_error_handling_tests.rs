...
fn test_error_codes_and_categories() {
    ...
    assert_eq!(uniform_err.category(), ErrorCategory::DistributionValidation);
...
fn test_error_context_builder_methods() {
    let mut err = FugueError::invalid_parameters(
        "TestDist", 
        "invalid parameter", 
    ...
    err = err.with_context("param", "value")
...
fn test_comprehensive_distribution_validation() {
    ...
    // Normal distribution
    assert_eq!(Normal::new(f64::NAN, 1.0).unwrap_err().code(), ErrorCode::InvalidMean);
    assert_eq!(Normal::new(0.0, -1.0).unwrap_err().code(), ErrorCode::InvalidVariance);
    
    // Bernoulli distribution  
    assert_eq!(Bernoulli::new(-0.5).unwrap_err().code(), ErrorCode::InvalidProbability);
    assert_eq!(Bernoulli::new(1.5).unwrap_err().code(), ErrorCode::InvalidProbability);
    
    // Beta distribution
    assert_eq!(Beta::new(-1.0, 2.0).unwrap_err().code(), ErrorCode::InvalidShape);
    assert_eq!(Beta::new(1.0, -2.0).unwrap_err().code(), ErrorCode::InvalidShape);
    
    // Gamma distribution
    assert_eq!(Gamma::new(-1.0, 2.0).unwrap_err().code(), ErrorCode::InvalidShape);
    assert_eq!(Gamma::new(1.0, -2.0).unwrap_err().code(), ErrorCode::InvalidRate);
    
    // Exponential distribution
    assert_eq!(Exponential::new(-1.0).unwrap_err().code(), ErrorCode::InvalidRate);
    
    // Uniform distribution
    assert_eq!(Uniform::new(5.0, 1.0).unwrap_err().code(), ErrorCode::InvalidRange);
    assert_eq!(Uniform::new(f64::NAN, 1.0).unwrap_err().code(), ErrorCode::InvalidRange);
    
    // Poisson distribution
    assert_eq!(Poisson::new(-1.0).unwrap_err().code(), ErrorCode::InvalidRate);
    
    // Categorical distribution
    assert_eq!(Categorical::new(vec![]).unwrap_err().code(), ErrorCode::InvalidProbability);
    assert_eq!(Categorical::uniform(0).unwrap_err().code(), ErrorCode::InvalidCount);
}
...
fn test_error_descriptions() {
    ...
    assert_eq!(ErrorCode::InvalidMean.description(), "Distribution mean parameter is invalid");
    assert_eq!(ErrorCode::InvalidVariance.description(), "Distribution variance/scale parameter is invalid");
    assert_eq!(ErrorCode::TraceAddressNotFound.description(), "Address not found in trace");
    assert_eq!(ErrorCode::TypeMismatch.description(), "Type mismatch in trace value");
    
    // Test category descriptions
    assert_eq!(ErrorCode::InvalidMean.category(), ErrorCategory::DistributionValidation);
    assert_eq!(ErrorCode::NumericalOverflow.category(), ErrorCategory::NumericalComputation);
    assert_eq!(ErrorCode::TraceAddressNotFound.category(), ErrorCategory::TraceManipulation);
}
