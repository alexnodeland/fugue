...
fn test_extreme_normal_parameters() {
    ...
    let small_sigma = Normal { mu: 0.0, sigma: 1e-10 };
    ...
    let large_sigma = Normal { mu: 0.0, sigma: 1e10 };
...
fn test_invalid_distribution_parameters() {
    ...
    let invalid_normal = Normal { mu: 0.0, sigma: -1.0 };
    ...
    let invalid_beta = Beta { alpha: -1.0, beta: 2.0 };
...
fn test_smc_weight_normalization() {
    ...
    let mut particles = vec![
        Particle { trace: Trace::default(), weight: 0.0, log_weight: -1000.0 },
        Particle { trace: Trace::default(), weight: 0.0, log_weight: 0.0 },
        Particle { trace: Trace::default(), weight: 0.0, log_weight: -500.0 },
    ];
...
fn test_categorical_edge_cases() {
    ...
    let unnorm_cat = Categorical { probs: vec![2.0, 3.0, 5.0] }; // Sum = 10
    assert!(unnorm_cat.sample(&mut rng).is_nan());
    
    // Negative probabilities
    let neg_cat = Categorical { probs: vec![0.5, -0.2, 0.7] };
    assert!(neg_cat.sample(&mut rng).is_nan());
    
    // Valid categorical
    let valid_cat = Categorical { probs: vec![0.3, 0.3, 0.4] };
...
fn test_conjugate_normal_inference() {
    ...
    let sample_var = mu_samples.iter()
    ...
    println!("Sample mean: {:.4}, Analytical: {:.4}", sample_mean, analytical_mean);
    println!("Sample var: {:.4}, Analytical: {:.4}", sample_var, analytical_var);
    
    // Very lenient bounds for MCMC convergence (just testing basic functionality)
    assert!((sample_mean - analytical_mean).abs() < 1.0, "Mean estimate too far from analytical solution");
    assert!((sample_var - analytical_var).abs() < 0.3, "Variance estimate too far from analytical solution");
}
