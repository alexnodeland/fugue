...
use fugue::inference::mh::{
    adaptive_mcmc_chain, adaptive_single_site_mh, DiscreteWalkProposal, FlipProposal,
    GaussianWalkProposal, ProposalStrategy, UniformCategoricalProposal,
};
...
fn test_type_safe_mh_preserves_f64_type() {
    ...
    let model_fn = || sample(addr!("mu"), Normal::new(0.0, 1.0).unwrap()).map(|mu| mu);
    ...
    for _ in 0..10 {
        let (_, new_trace) =
            adaptive_single_site_mh(&mut rng, &model_fn, &current_trace, &mut adaptation);
...
fn test_type_safe_mh_preserves_bool_type() {
    ...
    let model_fn = || sample(addr!("coin"), Bernoulli::new(0.7).unwrap()).map(|coin| coin);
...
fn test_type_safe_mh_preserves_u64_type() {
    ...
    let model_fn = || sample(addr!("count"), Poisson::new(3.0).unwrap()).map(|count| count);
...
fn test_mixed_type_model_mcmc() {
    ...
    let samples = adaptive_mcmc_chain(
        &mut rng, model_fn, 20, // Small number for test
        5,  // Brief warmup
    );
...
fn test_type_safe_proposal_edge_cases() {
    ...
    let (_, new_trace) = adaptive_single_site_mh(&mut rng, &model_fn, &trace, &mut adaptation);
...
fn test_adaptation_with_type_safe_mh() {
    ...
    let model_fn = || {
        sample(addr!("theta"), Normal::new(0.0, 1.0).unwrap()).bind(|theta| {
            observe(addr!("y"), Normal::new(theta, 0.1).unwrap(), 2.0).map(move |_| theta)
        })
    };
    ...
    for _ in 0..n_steps {
        ...
        assert!(matches!(
            current_trace.choices[&addr!("theta")].value,
            ChoiceValue::F64(_)
        ));
    }
...
fn test_mh_with_no_random_variables() {
    ...
    let (result, trace) =
        adaptive_single_site_mh(&mut rng, &model_fn, &empty_trace, &mut adaptation);
...
fn test_integration_type_safe_bayesian_regression() {
    ...
    let model_fn = || {
        ...
        // f64 parameters
        sample(addr!("slope"), Normal::new(0.0, 1.0).unwrap()).bind(|slope| {
            sample(addr!("intercept"), Normal::new(0.0, 1.0).unwrap()).bind(move |intercept| {
                // bool parameter for model selection
                sample(addr!("use_noise"), Bernoulli::new(0.8).unwrap()).bind(move |use_noise| {
                    // u64 parameter for robustness (number of outliers to expect)
                    sample(addr!("n_outliers"), Poisson::new(0.5).unwrap()).bind(
                        move |n_outliers| {
                            // usize parameter for noise model selection
                            sample(
                                addr!("noise_model"),
                                Categorical::new(vec![0.7, 0.3]).unwrap(),
                            )
                            .bind(move |noise_model| {
                                ...
                                let mut result =
                                    pure((slope, intercept, use_noise, n_outliers, noise_model));
                                for (i, (&x, &y)) in x_data.iter().zip(y_data.iter()).enumerate() {
                                    let y_pred = slope * x + intercept;
                                    result = result.bind(move |res| {
                                        observe(
                                            addr!("y", i),
                                            Normal::new(y_pred, final_noise).unwrap(),
                                            y,
                                        )
                                        .map(move |_| res)
                                    });
    ...
    // Run adaptive MCMC
    let samples = adaptive_mcmc_chain(
        &mut rng, model_fn, 100, // Reasonable number for integration test
        50,  // Warmup
    );
    ...
    let use_noise_flags: Vec<bool> = samples
        .iter()
        .filter_map(|(_, trace)| trace.get_bool(&addr!("use_noise")))
        .collect();

    let n_outliers_counts: Vec<u64> = samples
        .iter()
        .filter_map(|(_, trace)| trace.get_u64(&addr!("n_outliers")))
        .collect();

    let noise_models: Vec<usize> = samples
        .iter()
    ...
    for (i, &slope) in slopes.iter().enumerate() {
        ...
        if use_noise {
            // bool works naturally in conditionals
...
fn test_demonstrates_distribution_awareness_limitation() {
    ...
    let beta_model = || {
        ...
        sample(addr!("p"), Beta::new(2.0, 2.0).unwrap()).map(|p| p)
    };

    let normal_model = || {
        // Normal distribution is unbounded, so Gaussian proposals are appropriate
        sample(addr!("mu"), Normal::new(0.0, 1.0).unwrap()).map(|mu| mu)
    };
    ...
    // Extract values
    let beta_values: Vec<f64> = beta_samples
        .iter()
        .filter_map(|(_, trace)| trace.get_f64(&addr!("p")))
        .collect();

    let normal_values: Vec<f64> = normal_samples
        .iter()
