...
use fugue::runtime::memory::{CowTrace, TraceBuilder, TracePool};
...
fn bench_trace_builder(c: &mut Criterion) {
    ...
    for &size in &[10, 100, 1000, 5000] {
        ...
        group.bench_with_input(BenchmarkId::new("mixed_types", size), &size, |b, &size| {
            b.iter(|| {
                let mut builder = TraceBuilder::new();
                for i in 0..size {
                    let addr = addr!("mixed", i);
                    match i % 4 {
                        0 => builder.add_sample(
                            black_box(addr),
                            black_box(i as f64),
                            black_box(-0.5),
                        ),
                        1 => builder.add_sample_bool(
                            black_box(addr),
                            black_box(i % 2 == 0),
                            black_box(-0.693),
                        ),
                        2 => builder.add_sample_u64(
                            black_box(addr),
                            black_box(i as u64),
                            black_box(-1.5),
                        ),
                        3 => builder.add_sample_usize(
                            black_box(addr),
                            black_box(i % 3),
                            black_box(-1.1),
                        ),
                ...
                b.iter_batched(
                    ...
                    |mut pool| {
                        ...
                        for i in 0..num_traces {
                            ...
                            for j in 0..5 {
                                trace.insert_choice(
                                    Address(format!("iter#{}/param#{}", i, j)),
                                    ChoiceValue::F64(j as f64),
                                    -0.5,
                                );
                            }
...
fn bench_cow_trace(c: &mut Criterion) {
    ...
    for &size in &[10, 100, 500, 1000] {
        ...
        group.bench_with_input(BenchmarkId::new("first_write", size), &size, |b, _| {
            b.iter_batched(
                || base_cow.clone(),
                |mut cow| {
                    cow.insert_choice(
