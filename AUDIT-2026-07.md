...
**Superseded (2026-09, FG-N1):** the density probe at `−1.0` introduced by this fix was itself value-dependent (`current > 0 && log_prob(−1) == −inf`) and confined chains on bounded priors whose support contains negatives but not `−1` (e.g. `Uniform(−0.5, 0.5)`). Proposal kinds are now read from `Distribution::support()` (`Support::{Real, Positive, Bounded}`) via `proposal_kind_for_support`, with no probe and no per-address cache. The FG-42 regression test still passes.
...
**Follow-up (2026-09, FG-N8):** mdbook + its plugins and cargo-llvm-cov are no longer `[dev-dependencies]` at all (they are `cargo install`ed binaries; 461 → 99 lock packages), and the unused `clap`/`proptest` went with them. The MSRV job's `sed` stays as a cheap guard for the one remaining dev-dependency (criterion); its comment was updated.
